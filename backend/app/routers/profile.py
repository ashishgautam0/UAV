...
def _render_application_prompt(template, settings, jobs, resume, page_url, resume_url):
    ...
    hr_step = (
        ...
        "1. For each eligible job in this fixed batch, first verify its persisted Tracker record "
        "by exact posting URL and company/role. Open that record from Tracker; never guess a "
        "Tracker ID from the scraped-job ID. Do not generate or send HR email before tracking.\n"
        ...
        "on the same Tracker record and verify completion persists. If logging fails, retry "
