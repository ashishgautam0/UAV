...
E · Personalization angles: 2–3 specific hooks (from company research + JD) the outreach should use. If the profile shows an AWS certification and the JD touches AWS/cloud, make the certification the lead angle — it's a rare differentiator. Likewise, if this is a research role (research engineer / research scientist / applied scientist), lead with the M.Tech in AI and the peer-reviewed publication, plus the closest applied-research work in the profile (fine-tuning, low-resource ASR / language modelling, held-out evaluation with real metrics) — that combination is the differentiator there. Never claim a PhD or a top-tier ML-venue paper the profile does not show.
